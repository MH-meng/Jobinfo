class Pagination(object):
    def __init__(self, totleCount, currentPage, href, perPageItemNum=28, maxPageNum=11):
    ...
    def page_str(self):
        page_list = []
        first = "<li><a href='%s?p=1'>首页</a></li>" % self.href
        page_list.append(first)

        if self.current_page <= 1:
            prev = "<li><a href='#'>上一页</a></li>"
        else:
            prev = "<li><a href='%s?p=%s'>上一页</a></li>" % (self.href, self.current_page - 1)
        page_list.append(prev)

        for i in self.page_num_range():
            if i == self.current_page:
                temp = "<li class='active'><a href='%s?p=%s'>%s</a></li>" % (self.href, i, i)
            else:
                temp = "<li><a href='%s?p=%s'>%s</a></li>" % (self.href, i, i)
            page_list.append(temp)

        if self.current_page >= self.num_pages:
            nex = "<li><a href='#'>下一页</a></li>"
        else:
            nex = "<li><a href='%s?p=%s'>下一页</a></li>" % (self.href, self.current_page + 1)
        page_list.append(nex)

        final = "<li><a href='%s?p=%s'>尾页</a></li>" % (self.href, self.num_pages)
        page_list.append(final)

        return ''.join(page_list)


class PaginationG(object):
    def __init__(self, totleCount, currentPage, href, perPageItemNum=26, maxPageNum=11):
        self.totle_count = totleCount  # 数据总个数
        self.href = href
        try:
            v = int(currentPage)  # 当前页
